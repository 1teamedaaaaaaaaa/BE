...
import com.hoppin.infra.mail.service.EmailService;
...
public class InternalAiController {
    ...
    public AnalysisResponseDto analyze(
        ...
        AnalysisMailInfo mailInfo =
                promotionAnalysisService.getAnalysisMailInfo(promotionId, analysisJobId);

        if (mailInfo.email() != null && !mailInfo.email().isBlank()) {
            emailService.sendAnalysisResult(
                    mailInfo.email(),
                    mailInfo.musicianName(),
                    mailInfo.albumName(),
                    LocalDate.now(),
                    mailInfo.reportImageUrl(),
                    mailInfo.detailPageUrl()
            );
        }
